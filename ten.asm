...
half BYTE ".5"
playerHalfBoolean BYTE ?
dealerHalfBoolean BYTE ?
.code
ten PROC USES ebx edx ecx edi
	call Randomize
Start:
	; 初始化
	mov ebx, 0
	...
	mov playerHalfBoolean, 0
	mov dealerHalfBoolean, 0
...
;亂數產生莊家的牌--------------------------------------------
dealerRandom:
	.if  dealerSum >= 16	;若莊家牌的點數超過16則不繼續拿牌
		jmp	 PlayerRandom
	.elseif counter >=5		;若莊家的牌數超過5張則不繼續拿牌
...
PlayerRandom:
	.if  PlayerSum >= 1		 ;有一張牌之後每一次都問玩家還要不要拿牌
		.if PlayerSum <= 21
			jmp  select1 
		.endif
	.endif
	.if  PlayerSum > 21		;若玩家牌的點數超過21則跳出
		jmp  PrintFirstDealer 
	.endif
...
InttoStr:           
	...
	.IF playerHalfBoolean > 0
		add PlayerStrPosition.x, 2
		INVOKE WriteConsoleOutputCharacter,
		  outputHandle,     
		  ADDR half, 
		  SIZEOF half,        
...
InttoStr1:           
	...
	.IF dealerHalfBoolean > 0
		add dealerStrPosition.x, 2
		INVOKE WriteConsoleOutputCharacter,
		  outputHandle,     
		  ADDR half, 
		  SIZEOF half,        
